...
Distributed representation of clustering may be better than the one-hot encoding provided by K-means clustering.

Before deep learning, to train nonlinear models we used the kernel trick in combination with a linear model. Deep learning is a scalable way of training nonlinear models on large datasets.
...
Most widely used prior is smoothness prior, function we learn should not change very much within a small region. This prior only work well if we have enough data points and that the underlying function is smooth too.
